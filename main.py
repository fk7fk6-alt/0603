...
# 🔮 사주 분석 데이터 셋 (외부 라이브러리 없이 랜덤/해싱 조합으로 고유 결과 생성)
saju_titles = [
...
saju_details = {
    ...
    "🛡️ 듬직하고 영리한 우리 집 대장님, '용맹무쌍 호위무사' 팔자": "사주에 용맹함과 충성심이 가득합니다! 🎖️ 작은 소리도 놓치지 않고 집을 지키며, 보호자가 우울해 보일 때 귀신같이 다가와 위로해 주는 든든한 보디가드네요. 책임감이 강하고 영리하여 온 가족의 정신적 지주 역할을 합니다. 🦅",
...
if st.sidebar.button("🔮 댕사주 확인하기! 🔮", use_container_width=True):
    ...
    # 외부 라이브러리(matplotlib 등) 없이 streamlit 내장 차트로 화려하게 표현
    chart_data = {
        "🌳 목(나무 - 활동성)": wood,
        "🔥 화(불 - 애교와 열정)": fire,
        "🌱 토(흙 - 듬직함)": earth,
